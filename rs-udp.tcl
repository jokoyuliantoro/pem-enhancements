...
when CLIENT_ACCEPTED {
  ...
  if { $srv_ptr_name ne "No_PTR" } {
    ...
    unset -nocomplain a c cmd ptracte
  ...
  after 5000 -periodic {
    ...
    if { "[lindex $lstpkt 0][lindex $lstpkt 1]" ne "[lindex $curpkt 0][lindex $curpkt 1]" } {
      ...
      set lstpkt $curpkt
      HSL::send $hsl "UDP\$\$0\$\$$action\$\$$tccat\$\$$tcapp\$\$$tcres\$\$$srv_ptr_name\$\$$asorigin\$\$[IP::client_addr]\$\$[UDP::client_port]\$\$[IP::local_addr]\$\$[UDP::local_port]\$\$$ipstat\$\$$sni\$\$$sni_lookup\$\$$cipher\$\$$httpinfo\$\$$httpstatus\$\$$httpsinfo\$\$$subsid\$\$$rat\$\$$nas\$\$$imei\$\$$towerid\$\$$callingsid\$\$$calledsid"
    }
  }
}
...
when CLIENT_CLOSED {
  ...
  set dbo [expr {[lindex $curpkt 3]-[lindex $lstpkt 3]}]
  set ipstat "$dpi\$\$$dpo\$\$$dbi\$\$$dbo\$\$[lindex $curpkt 4]"
  HSL::send $hsl "UDP\$\$1\$\$$action\$\$$tccat\$\$$tcapp\$\$$tcres\$\$$srv_ptr_name\$\$$asorigin\$\$[IP::client_addr]\$\$[UDP::client_port]\$\$[IP::local_addr]\$\$[UDP::local_port]\$\$$ipstat\$\$$sni\$\$$sni_lookup\$\$$cipher\$\$$httpinfo\$\$$httpstatus\$\$$httpsinfo\$\$$subsid\$\$$rat\$\$$nas\$\$$imei\$\$$towerid\$\$$callingsid\$\$$calledsid"
}
